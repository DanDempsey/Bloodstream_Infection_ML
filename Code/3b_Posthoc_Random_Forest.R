##### Code for Random Forest Supplemental Graphs
##### Daniel Dempsey
...
setwd( 'Output/Model_Fits/RandomForest' )
...
X <- res$cv_results$X
med_shap_list <- split( t(med_shap), rep( 1:ncol(X), each = nrow(X) ) )
names( med_shap_list ) <- colnames( X )
shap_contrib <- do.call( 'cbind', med_shap_list ) %>% as.data.table

png( 'Geometric_Median_Shapley_Values_full.png', width = 700, height = 600 )
...
png( 'Repeated_Shaps.png', width = 700, height = 600 )
...
png( 'Repeated_ROC.png', width = 700, height = 600 )
...
CV_var_rel <- function( x ) {
  ...
  png( paste0('CE_', filename, '.png'), width = 700, height = 600 )
  ...
  #png( paste0('AUC_', filename, '.png'), width = 700, height = 600 )
...
all_shaps_val$boot_ind <- rep( 1:B, each = n )
...
for ( i in 1:N ) {
  ...
  colnames( dat )[1] <- 'val'
  obs_ind <- order( X[, cn] )
  dat_list <- split( dat, rep( 1:B, each = n ) )
  reorder_list <- lapply( dat_list, function(x) { x$val[obs_ind] } )
  hm_dat <- data.frame( val = unlist(reorder_list), obs_ind = rep(1:n, B), boot_ind = rep(1:B, each = n) )
  ...
  png( paste0('Heatmaps/shap_heatmap_', i, '.png'), width = 700, height = 600 )
  print( hm )
  dev.off()
...
vio_plot_fun <- function( z ) {
  ...
  ggplot(top_dat, aes(x = Exposure_lab, y = get(z))) +
    ...
    geom_sina( aes(x = Exposure_lab, y = get(z), color = use_cols), method = "counts", maxwidth = 0.7, 
               alpha = 0.7 ) + theme(legend.position = "none") + ggtitle( z ) + ylab( '' ) + xlab( '' )
...
### Numerical Table
hd_dat <- read_csv( '../../../Data/hd_bsi.csv' )
...
age_split <- list()
age_split[[1]] <- split( hd_dat$Age, hd_dat$Exposure )
...
#mod_split <- list()
#mod_split[[1]] <- split( hd_dat$MonthsSinceStartingDialysis, hd_dat$Exposure )
...
sex <- hd_dat$Sex
sex_cohort[2:3, 2:3] <- sex_props <- table( sex, hd_dat$Exposure ) %>% as.matrix
...
ckd_cohort$Variable <- c( 'Aetiology of CKD', 'Chronic Glomerulonephritis', 'Ischaemic Nephrology',
                          ...
                          'Unknown', '' )
ckd_dat <- split( hd_dat[, grepl('AetiologyofCKD.', colnames(hd_dat))], hd_dat$Exposure )
...
# Previous Transplant
pt_cohort <- matrix( '', nrow = 4, ncol = 4 ) %>% as.data.frame
names( pt_cohort ) <- cat_names
pt_cohort$Variable <- c( 'Previous Transplant', 'Yes', 'No', '' )
pt_cohort[2:3, 2:3] <- pt_props <- table( hd_dat$TransplantEver, hd_dat$Exposure, useNA = 'ifany' ) %>% as.matrix
pt_cohort[1, 4] <- round( prop.test(pt_props)$p.value, 2 )

# Immunosuppressive Therapy
ist_cohort <- matrix( '', nrow = 4, ncol = 4 ) %>% as.data.frame
names( ist_cohort ) <- cat_names
ist_cohort$Variable <- c( 'Immunosuppressive Therapy', 'Yes', 'No', '' )
ist_cohort[2:3, 2:3] <- ist_props <- table( hd_dat$CurrentImmunosuppressiveRx, hd_dat$Exposure, useNA = 'ifany' )
ist_cohort[1, 4] <- round( prop.test( ist_props )$p.value, 2 )
...
va_dat <- split( hd_dat[, grepl('VascularAccess.', colnames(hd_dat))], hd_dat$Exposure )
