##### Code for Further Graphs and Analysis from the XGBoost fit
##### Daniel Dempsey
...
quantile( train_auc, probs = 0.5 )
quantile( test_auc, probs = 0.5 )

#t.test( train_auc, mu = 0.5 )
#t.test( test_auc, mu = 0.5 )
...
for ( i in 1:ncol(top_shaps) ) {
  ...
  png( paste0('AUC_SHAP_', nm, '.png'), width = 700, height = 600 )
...
age_split <- list()
age_split[[1]] <- split( hd_dat$Age, Exposure )
...
age_cohort
lapply( age_split, sapply, length )
...
#mod_split <- list()
#mod_split[[1]] <- split( hd_dat$MonthsSinceStartingDialysis, Exposure )
...
write.csv( age_cohort, 'Numerical_table.csv' )
...
sex <- hd_dat$Sex
sex_cohort[2:3, 2:3] <- sex_props <- table( sex, Exposure ) %>% as.matrix
...
ckd_cohort$Variable <- c( 'Aetiology of CKD', 'Chronic Glomerulonephritis', 'Ischaemic Nephrology',
                          ...
                          'Unknown', '' )
ckd_dat <- split( hd_dat[, grepl('AetiologyofCKD.', colnames(hd_dat))], Exposure )
...
pt_cohort[2:3, 2:3] <- pt_props <- table( hd_dat$TransplantEver, Exposure, useNA = 'ifany' ) %>% as.matrix
...
ist_cohort[2:3, 2:3] <- ist_props <- table( hd_dat$CurrentImmunosuppressiveRx, Exposure, useNA = 'ifany' )
...
va_dat <- split( hd_dat[, grepl('VascularAccess.', colnames(hd_dat))], Exposure )
