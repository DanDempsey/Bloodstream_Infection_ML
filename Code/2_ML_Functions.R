...
fit_fun <- function( X, y, param_grid, model_mode = 1, outer_folds = nrow(X), 
  ...
  setwd( directory )
  if ( model_mode == 1 ) {
    sub_directory <- 'XGBoost'
  }
  else if ( model_mode == 2 ) {
    sub_directory <- 'RandomForest'
  }
  else if ( model_mode == 3 ) {
    sub_directory <- 'LASSO'
  }
  else {
    stop( "model_mode must be 1 (for xgboost), 2 (for random forest), or 3 (for LASSO)." )
  }
  ...
  if ( missing(param_grid) ) {
    if( !(model_mode == 3) ) {
      stop( "param_grid must be supplied if not using LASSO.")
    }
    else {
      prefit_weight <- rep( 1, length(y) )
      prefit_weight[y == 1] <- sum( 1 - y ) / sum( y )
      lambda_max <- glmnet:::get_start( as.matrix(X_all_imp), y, weights = prefit_weight, 
                                        family = binomial(), intercept = TRUE, is.offset = FALSE, 
                                        offset = NULL, exclude = NULL, vp = rep(1, ncol(X_all_imp)), 
                                        alpha = 1 )$lambda_max
      lambda_seq <- glmnet( as.matrix(X_all_imp), y, weights = prefit_weight, 
                            family = "binomial" )$lambda
      param_grid <- list( lambda = lambda_seq )
    }
  }
  ...
  for ( i in 1:outer_folds ) {
    ...
    for ( j in 1:inner_folds ) {
      ...
      if ( model_mode == 3 ) { # LASSO
        cv_fit <- lapply( all_param_grid_split, function(z) {
          glmnet( X_inner_train, factor(y_inner_train), family = "binomial",
                  weights = weight_cv, lambda = z$lambda )
        } )
        test_preds <- lapply( cv_fit, predict, newx = X_inner_test, type = 'response' )
      }
    ...
    if ( model_mode == 2 ) { # Random Forest
      outer_fit <- randomForest( x = X_outer_train, y = factor(y_outer_train),
                                 ...
                                 mtry = best_params$mtry, weights = weight_cv )
    ...
    if ( model_mode == 3 ) {
      outer_fit <- glmnet( as.matrix(X_outer_train), factor(y_outer_train), family = "binomial",
                           weights = weight_cv, lambda = best_params$lambda )
      outer_preds_train <- predict( outer_fit, newx = X_outer_train, type = 'response' )
      outer_preds_test <- predict( outer_fit, newx = X_outer_test, type = 'response' )
      colnames( outer_preds_train ) <- colnames( outer_preds_test ) <- 'preds'
    }
    ...
    if ( !(model_mode == 3) ) {
      ...
      outer_shaps_all[[i]] <- outer_fit$beta
    }
  ...
  # Variable importance
  if ( !(model_mode == 3) ) { # Shapley geometric median
    ...
    var_prop <- apply( var_shrink, 2, mean ) %>% sort %>% rev
    hi <- which( names(var_prop) == 'random_num' )
    col_seq <- rep( mycol[1], length(var_prop) )
    col_seq[hi] <- mycol[2]
    
    pdf( "LASSO_Selection.pdf", height = 10, width = 10 )
    par( mar = c(10, 4, 4, 2) + 0.1 )
    barplot( var_prop, las = 2, col = col_seq, main = 'Proportion of LASSO Selection' )
  ...
  if ( model_mode == 3 ) {
    res <- list( accuracy_train = outer_preds_train_dat, accuracy_test = outer_preds_test_dat, 
                 coefs = outer_shaps_all, chosen_parameters = best_params_dat )
  }
  else {
    res <- list( accuracy_train = outer_preds_train_dat, accuracy_test = outer_preds_test_dat, 
